...
```{r correlation between gdp and CO2 emissions coloured by time frame}
Our_co2_dataset %>%
...
```{r create map coloured by responsibility value}
merged_data %>% 
  filter(year == 2018) %>% 
...
```{r create map coloured by CO2 per capita}
merged_data %>% 
  filter(year == 2018) %>% 
...
```{r create interactive leaflet map coloured by responsibility value}
...
```{r create interactive leaflet map coloured by CO2 per capita}
...
```{r create interactive leaflet map coloured by CO2}
...
```{r animate map and save animation as an mp4 file}
