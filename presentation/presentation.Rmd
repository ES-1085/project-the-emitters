...
```{r correlation between gdp and CO2 emissions coloured by time frame, fig.alt = "This plot show the correlation between CO2 emmisions and GDP. We see a general trend of CO2 and gdp increasing over time."}
Our_co2_dataset %>%
...
```{r create_map_coloured_by_responsibility_value, fig.alt = "Figure showing a map of the world coloured by 2018 reponcibility value. Canada, the USA, Saudia Arabia, Australia, and Norway seem to have the highest responcibility values."}
...
```{r create_map_coloured_by_CO2_per_capita, fig.alt="Figure showing a map of the world coloured by 2018 CO2 emissions per capita. Kazakhstan, Saudi Arabia, Mongolia, United States, Canada, and Australia seem to have the highest CO2 emissions per capita."}
# merged_data %>% 
#   filter(year == 2018) %>% 
...
```{r  Create_map_coloured_by_CO2, fig.alt="Figure showing a map of the world coloured by 2018 CO2 emissions. The USA and China have by far the greatest CO2 emissions."}
# merged_data %>% 
#   filter(year == 2018) %>% 
# ggplot(aes(fill = co2)) +
#     geom_sf() +
...
```{r create_interactive_leaflet_map_coloured_by_responsibility_value, fig.alt="Figure showing an inteactive map of the world coloured by 2018 reponcibility value. Canada, the USA, Saudia Arabia, Australia, and Norway seem to have the highest responcibility value."}
...
```{r create_interactive_leaflet_map_coloured_by_CO2_per_capita, fig.alt="Figure showing an interactive map of the world coloured by 2018 CO2 emissions per capita. Kazakhstan, Saudi Arabia, Mongolia, United States, Canada, and Australia seem to have the highest CO2 emissions per capita."}
...
```{r create_interactive_leaflet_map_coloured_by_CO2, fig.alt="Figure showing a map of the world coloured by 2018 CO2 emissions. The USA and China have by far the greatest CO2 emissions."}
...
# Co2_vs_pop_scatter_log <- Only_Countries_CO2 %>%   
#   filter(year > 1800) %>% 
#   filter(continent != "Antarctica") %>% 
#   filter(continent != "Seven seas (open ocean)") %>% 
#   ggplot(aes(x = population, y = co2)) + 
...
# animate(Co2_vs_pop_scatter_log, renderer = gifski_renderer(), fps = 4)
# 
# anim_save(filename = "Co2_vs_pop_scatter_log.gif", animation = Co2_vs_pop_scatter_log, path = NULL)
```

```{r create_histogram}
# animate map and save animation as an mp4 file
