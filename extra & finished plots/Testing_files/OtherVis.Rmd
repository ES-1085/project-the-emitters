...
```
```{r}
animate(timePlot2, renderer = gifski_renderer(), end_pause = 20)

anim_save(filename = "timePlotTest.gif", animation = timePlot2, path = NULL)
```
...
```
```{r}
animate(timeCo2, renderer = gifski_renderer(), end_pause = 20)

anim_save(filename = "timeCo2.gif", animation = timeCo2, path = NULL)
```
```
